...
#ifdef DMDDLL_EXPORTS
#define DMD_DLL_API __declspec(dllexport)
#else
#define DMD_DLL_API __declspec(dllimport)
#endif
...
/* spostate in measure.h

struct DMD {
	hid_device *handle;
	struct DmdTx_Patterns* pattern;
	int szPattern;
	int repeat;
};
...
DMD_DLL_API void DmdTx_writeOnFile(char *fileName, unsigned char *data, int size);

/** same of writeOnFile but with (int) data variable (used only in defSequence) */
DMD_DLL_API void DmdTx_writeOnFile_int(char *fileName, int data [], int size);

/** This function converts a number "a" into a bit string of  length "l"
note: should be a < 2^l (it is not verified) */
DMD_DLL_API char * DmdTx_convlen(int a, int l);
...
DMD_DLL_API int * DmdTx_bitsToBytes(const char *a, const int size);

/** function to write data in the DMD buffer
for more informations look at the ti DLPC900 Programmer's Guide */
DMD_DLL_API int DmdTx_talkDMD_char(hid_device *handle, const char mode, const char sequencebyte, const char com1, const char com2, const unsigned char *data, const int sizeData);
...
DMD_DLL_API int DmdTx_talkDMD_int(hid_device *handle, const char mode, const char sequencebyte, const char com1, const char com2, const int *data, const int sizeData);

/** changes the internal image processing function of the DMD
see DMD Programmer's Guide pag. 58 */
DMD_DLL_API void DmdTx_changeMode(hid_device *handle, int mode);
...
DMD_DLL_API void DmdTx_stopSequence(hid_device *handle);

/** function tells the DMD to start reproducing the pattern sequence stored in memory
see DMD Programmer's Guide pag. 69 */
DMD_DLL_API void DmdTx_startSequence(hid_device *handle);
...
DMD_DLL_API void DmdTx_reset(hid_device *handle);

/** retrieve the error number code from the DMD
see DMD Programmer's Guide pag. 23 */
DMD_DLL_API void DmdTx_checkForErrors(hid_device *handle);

/** retrieve the error message string from the DMD
see DMD Programmer's Guide pag. 23 */
DMD_DLL_API void DmdTx_checkErrorMessage(hid_device *handle);
...
DMD_DLL_API void DmdTx_configureLut(struct DmdTx_Patterns *pattern, int size, int rep);
...
DMD_DLL_API void DmdTx_mergeImages(unsigned char ***images, unsigned char ***res);
...
DMD_DLL_API void DmdTx_definePatterns(struct DmdTx_Patterns *pattern,const int index,const int exposure,const int bitdepth, const char *color,const int triggerIn,const int darkTime,const int triggerout,const int patInd,const int bitpos);
...
DMD_DLL_API void DmdTx_defSequence(struct DmdTx_Patterns *pattern, unsigned char ***matrixes, int *exposure, int *trigger_in, int dark_time, int *trigger_out, int repetition, const int size);

/** function to push data in LIFO structure */
DMD_DLL_API void DmdTx_push(struct DmdTx_Node **head, unsigned char data);
...
DMD_DLL_API void DmdTx_allocatePattern(struct DmdTx_Patterns *p, int nEl);

/** function for eRLE to verify if planes of mergedImages are equal
return 0 if they are not equal
return 1 if they are equal */
DMD_DLL_API int DmdTx_isRowEqual(const unsigned char *a, const unsigned char *b);

/** sets parameters (index and number of bytes) for image (group of patterns)
p. 75 DLPC900 Programmer's guide (initialize BMP Load) */
DMD_DLL_API void DmdTx_setBmp(struct DmdTx_Patterns *pattern, const int  index, const int size);

/** load (or load to Patterns) the encoded image to the DMD
p. 75-76 DLPC900 Programmer's guide */
DMD_DLL_API void DmdTx_bmpLoad(struct DmdTx_Patterns *pattern, const int index, const unsigned char *image, const int size);
...
DMD_DLL_API int DmdTx_newEncode2(unsigned char ***image, struct DmdTx_Node **n);

/** take data of the pattern from txt file */
DMD_DLL_API int DmdTx_takeFromTxt(struct DmdTx_Node **bitString);

/** function to create a pattern from a 24-bit bmp image (note: only one color channel is read) */
DMD_DLL_API void DmdTx_readBMP(char *filename, int *** image);

/** deallocates all the memory allocated in defPatterns containing the pattern and relative info */
DMD_DLL_API void DmdTx_deallocate(struct DMD *dmd);

/* basic mathematical functions */
DMD_DLL_API int DmdTx_pow2_i(const int exp);
DMD_DLL_API int DmdTx_pow_i(const int b, const int exp);    // these are in ordering.c
DMD_DLL_API int DmdTx_celing(const int a, const int b);
