...
void StartFlowMharp(void){
	...
	if(P.Flow.MultiThread){
    	...
    	rbuffer.count = 0;
		rbuffer.stop =0;
		rbuffer.data = calloc(MHARP_BUFFER_RING, sizeof(unsigned long));
		P.Flow.StopMultiThread=FALSE;

		// Initialize mutex and condition variable
    	InitializeCriticalSection(&rbuffer.mutex);
    	InitializeConditionVariable(&rbuffer.notEmpty); // Not needed?
    
    	// Create threads for acquiring and analyzing data
    	HANDLE acquireThread = CreateThread(NULL, 0, acquireData, &rbuffer, 0, NULL);
...
void StopFlowMharp(void){
	...
	if(P.Flow.MultiThread){ 
    	...
    	DeleteCriticalSection(&rbuffer.mutex);
