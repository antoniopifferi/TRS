...
//			}
	...
	P.Spc.Started=FALSE;
	P.Spc.Trash=TRUE;
	if((P.Spc.Type==SPC300)||(P.Spc.Type==SPC630)||(P.Spc.Type==SPC130)||(P.Spc.Type==HYDRA)) P.Meas.Stop=TRUE;
	if((P.Spc.Type==SPC300)||(P.Spc.Type==SPC630)||(P.Spc.Type==SPC130)||(P.Spc.Type==HYDRA)||(P.Spc.Type==TEST)) P.Spc.Format=SPC_SHORT;
...
void CheckJump(void){
	...
	switch (P.Jump.Loop){
		case LOOP1: delta=P.Loop[LOOP2].Num*P.Loop[LOOP3].Num; break;
		case LOOP2: delta=P.Loop[LOOP3].Num; break;
		case LOOP3: delta=1; break;
...
void GetDataHydra(void){
	...
	P.Spc.Overflow=FALSE;
	for(ib=0;ib<P.Num.Board;ib++){
		// TODO: ret=SPC_test_state(ib,&state);
		// TODO: if(ret<0) ErrHandler(ERR_SPC,ret,"SPC_test_state");
		// TODO: P.Spc.Overflow|=(state&SPC_OVERFLOW);
		//ret=HH_GetHistogram(HYDRA_DEV0,(unsigned int*)D.Buffer[ib],ib,0); if(ret<0) ErrHandler(ERR_HYDRA,ret,"HH_GetHistogram");
/*		do{
			ret=HH_GetHistogram(HYDRA_DEV0,DataHydra,ib,0); if(ret<0) ErrHandler(ERR_HYDRA,ret,"HH_GetHistogram");
			if(ret<0){
				Delay(0.1);
				sprintf(message,"trial no = %d",i_trial);
				MessagePopup ("ERROR ON GETTING",message);
				i_trial++;
				}
			}   
...
void ClearHydra(void){
	...
	do{
		...
		if(ret<0){
			//Delay(0.1);
			//sprintf(message,"trial no = %d",i_trial);
			//MessagePopup ("ERROR ON CLEARING",message);
			i_trial++;
			//for(ib=0;ib<P.Num.Board;ib++) InitHydra(ib);
			ret=HH_SetHistoLen(HYDRA_DEV0,HYDRA_LENCODE,&HistLen); if(ret<0) ErrHandler(ERR_HYDRA,ret,"HH_SetHistoLen");
			}
		}
	while((ret<0)&&(i_trial<10));
...
void AutoTrim(int Trim){
	...
	char is_any_SPC = ((P.Spc.Type==SPC300)||(P.Spc.Type==SPC630)||(P.Spc.Type==SPC130)||(P.Spc.Type==HYDRA));
...
void CloseStep(char Step){
	switch(P.Step[Step].Type){
		...
		case DELAYER: CloseDelayer(Step); break;
		case ADC: break;
		case AOTF_FREQ:	break;
		case AOTF_POW:	break;
...
void MoveStep(long *Actual,long Goal,char Step,char Wait,char Status){
	...
	switch (P.Step[Step].Type){
		...
		case AOTF_POW: SetVoltNI_USB6229(Step, Goal); break;*/
...
void InitPi(char Step){
	...
	char message[STRLEN];
	int com=P.Step[Step].Axis+1;
	sprintf(message,"Initializing PI Driver #%d on COM%d\n",Step+1,com);
    SetCtrlVal (hDisplay, DISPLAY_MESSAGE,message);
	//FlushInQ (com);
	//FlushOutQ (com);
	//CloseCom (com);
...
void NKTSendMessage(char Step,void *Value, unsigned char Dest, unsigned char Source, unsigned char Type,unsigned char Register, unsigned int DataType){
	...
	if (ComWrt (com, txBuffer, txBufferSize)!=txBufferSize)
		 {
...
void StartAdc(void){
	short status;
	double actualScanRate;
	//status=nidaqAIStart (P.Power.Adc.Task);
	status = AIStartAcquisition (POWER_ADC_BOARD,"4",10*1000,1000.0,-0.9,-1.0,&actualScanRate,&P.Power.Adc.Task);
    if(status!=0) Failure(GetDAQErrorString(status));
...
void StopAdc(void){
    ...
    status = AICheckAcquisition (P.Power.Adc.Task,&scanBacklog);
    if(status!=0) Failure(GetDAQErrorString(status));
	//**sprintf(message,"AICheckAcquisition = %d\n",scanBacklog);
    //**SetCtrlVal (hDisplay, DISPLAY_MESSAGE,message);
    status = AIReadAcquisition (P.Power.Adc.Task,scanBacklog,CONSECUTIVE_MODE,&scanBacklog,GROUP_BY_SCAN,P.Power.Adc.Data);
    if(status!=0) Failure(GetDAQErrorString(status));
    StdDev (P.Power.Adc.Data, 1000, &value, &stdev);
    P.Step[P.Power.Step].Actual=(long)(value*ADC_FACTOR);
